...
ln -s ~/.agents/skills/model-router ~/.claude/skills/model-router   # if Claude Code should see it
...
- `roster.md` — replace the author's subscriptions with yours. Keep the section headings.
- `config.json` — list the same products and models, so `limits.py` knows the keys:

```json
{
  "launchers": ["tmux", "shell", "subagent"],
  "products": {
...
`launchers` is your order of preference. Leave it out and it is detected: `orca` if installed, `shell`, `tmux` if installed, `subagent`. `free_until` marks a product dead after that date. `MODEL_ROUTER_HOME` moves the whole directory.

To make sure the agent reads the skill, add one line to your `CLAUDE.md` / `AGENTS.md`:

```md
Read the `model-router` skill before delegating work or choosing a model.
```
...
Standard library only, Python 3.9+. The parent starts children through `run`. `run` checks the records first and refuses to start a dead tier; if the child ends with limit wording, `run` records it and exits 75. The parent sees 75 and moves to the next tier.
...
python3 $L run swe -- devin --model swe-2-medium -p "..."   # start a headless child (--log FILE keeps its output)
...
- Supported for products named `claude` and `codex`; others are skipped. Claude's token comes from the macOS Keychain (`Claude Code-credentials`) or `~/.claude/.credentials.json`; Codex's from `~/.codex/auth.json`. Both usage APIs are private and may change — when they cannot be read, the grade is normal.
