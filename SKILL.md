...
計画・レビュー・昇格を決める前に `python3 ~/.agents/skills/model-router/limits.py budget` を見る。Claude と Codex の週枠を、ペース（週の経過割合 − 使用率）で **余り / 普通 / 節約** に分けて返す。取得できないときは普通。結果は 5 分キャッシュされ、判定は委譲のたびにやり直すので、格上げして枠が減れば自然に戻る。

余っていても実装・量産・裏取り・CSS 量産は SWE-2 のまま。余りは考える役に使う。Astra は Codex が余っていても UI 専用。Devin と Grok は対象外。

| Claude の状態 | 設計・Plan | SWE-2 からの昇格 | 詰まったデバッグ | 実装レビュー | リポジトリ探索 |
|---|---|---|---|---|---|
| 余り | Fable xHigh を常用 | 失敗 1 回で Claude へ | SWE-2 High が 1 回失敗したら Fable（Max を挟まない） | Opus 5 High 固定 | Sonnet 5 |
| 普通 | 役割表どおり | 役割表どおり | 役割表どおり | 役割表どおり | 役割表どおり |
| 節約 | Opus 5 High | 失敗 2 回。昇格先は Codex × Sol High | フォールバックは Sol High を先に | Sonnet 5。重要な差分だけ Opus 5 | Haiku 4.5 |

- モデル別の行（`opus` など）が出たら、そのモデルは製品全体と悪いほうの状態で扱う。
- Codex が余り：実装レビューのあと Sol High の二重レビューを常に行う。
- Codex が節約：計画のレビューを Claude × Opus 5（新セッション）へ回し、Astra を UI だけに残す。
